...
const AdminDashboard = () => {
  ...
  const [comp, setComp] = useState({
    ...
    primary_color_hex: '#133651',
    secondary_color_name: 'Accent Blue',
    secondary_color_hex: '#73D2E1',
    primary_font_name: 'Titillium Web',
    primary_font_link: 'https://fonts.google.com/specimen/Titillium+Web',
    secondary_font_name: 'Karla',
    secondary_font_link: 'https://fonts.google.com/specimen/Karla'
  });
  ...
  const submitAsset = async (e) => {
    ...
    if (!assetCompanyId || !assetName || !assetFile || !previewImages.length) {
    ...
    try {
      ...
      await api.post('/dashboard/asset', formData, {
  ...
  if (isLoading) {
    return (
      <div className="bg-light min-vh-100">
        <nav
          className="navbar navbar-dark p-3 px-5 d-flex justify-content-between shadow-sm placeholder-glow"
          style={{ background: 'linear-gradient(135deg, #1e3c72, #2a5298)' }}
        >
          <span
            className="placeholder col-2 rounded"
            style={{ height: '30px', backgroundColor: 'rgba(255,255,255,0.7)' }}
          ></span>
          <span
            className="placeholder col-1 rounded"
            style={{ height: '35px', backgroundColor: 'rgba(255,255,255,0.7)' }}
          ></span>
        </nav>
      ...
      <div className="bg-light min-vh-100" style={{ paddingBottom: '50px' }}>
        <nav className="navbar custom-navbar p-3 px-4 px-md-5 d-flex justify-content-between sticky-top">
          <div className="d-flex align-items-center">
            ...
            <h4 className="text-white brand-font m-0 fw-bold" style={{ letterSpacing: '0.5px' }}>
              Lacas Assets Admin
            </h4>
          </div>
          <button
            className="btn text-white btn-sm px-3 py-2 rounded-pill logout-btn fw-semibold"
            ...
            >
              ...
              <Tab eventKey="asset" title="📁 Upload Assets">
                <div className="animate__animated animate__fadeIn">
                  <AssetForm
                    companies={companies}
                    setAssetCompanyId={setAssetCompanyId}
                    assetName={assetName}
